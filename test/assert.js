const assert = {
  ...
  exists(testName, functionString, value) {
    ...
    if (value) {
      console.log(
        "\x1b[32m%s\x1b[0m",
        `${testNumber} - ${functionString} is defined - Passed!`
      );
    } else {
      console.log(
        "\x1b[31m%s\x1b[0m",
        `${testNumber} - ${functionString} is defined - Failed: ${functionString} was not properly defined.`
      );
    }
  },
  ...
  equals(testName, description, functionString, actualValue, expectedValue) {
    ...
    if (actualValue === expectedValue) {
      console.log(
        "\x1b[32m%s\x1b[0m",
        `${testNumber} - ${description} - Passed!`
      );
    } else {
      console.log(
        "\x1b[31m%s\x1b[0m",
        `${testNumber} - ${description} - Failed: ${functionString} returned ${actualValue} instead of ${expectedValue}.`
      );
    }
  },
  ...
  arrayEquals(
    ...
    if (arraysAreEqual(actualValue, expectedValue)) {
      ...
      console.log(
        "\x1b[31m%s\x1b[0m",
        `${testNumber} - ${description} - Failed: ${functionString} returned ${arrayToString(
          actualValue
        )} instead of ${arrayToString(expectedValue)}.`
      );
    }
  },
};
